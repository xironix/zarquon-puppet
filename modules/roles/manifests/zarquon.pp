class roles::zarquon {
 ...
 firewall { '202 allow 7442 (alt ssh port)':
  ...
  # WordPress - local
  nginx::resource::vhost { 'zarquon.esurient.local':
    ensure                  => present,
    ssl                     => 'true',
  ...
  nginx::resource::location { 'zarquon.esurient.local':
    ensure              => present,
    ssl                 => 'true',
    location            => '~ \.php$',
    www_root            => '/var/www/trollop.org/blog',
  ...
  # rutorrent - torrents.local
  nginx::resource::vhost { 'torrents.esurient.local':
    ensure                   => present,
    www_root                 => '/var/www/trollop.org/torrents',
    location_cfg_prepend     => {
      'auth_basic'           => '"Restricted"',
      'auth_basic_user_file' => '/var/www/trollop.org/torrents/torrents.local.pass',
    };
  }
  nginx::resource::location { 'torrents.esurient.local':
    ensure               => present,
    location             => '~ \.php$',
    www_root             => '/var/www/trollop.org/torrents',
    vhost                => 'torrents.local',
    location_cfg_append  => $fastcgi,
    location_cfg_prepend => {
      'auth_basic'           => '"Restricted"',
      'auth_basic_user_file' => '/var/www/trollop.org/torrents/torrents.local.pass',
